...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun)
        {
            ...
            {
                float spread = UnityEngine.Random.Range(-shotgunSpread, shotgunSpread);
                Quaternion bulletRotation = Quaternion.Euler(0f, 0f, director.eulerAngles.z + spread);
                SpawnBullet(director.position, bulletRotation);
            }
        }
        SpawnBullet(
            director.position,
            Quaternion.Euler(0f, 0f, director.eulerAngles.z)
        );
    }
